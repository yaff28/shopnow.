...
let cart            = safeParseJSON(‘sn_cart’, []);
let wishlist        = safeParseJSON(‘sn_wish’, []);
...
return n >= 1000 ? (n / 1000).toFixed(1) + ‘rb’ : n;
}
...
const d      = discPct(p.price, p.orig);
const div    = document.createElement(‘div’);
div.className = ‘p-card’;
div.innerHTML = `<div class="p-card-img"> ${p.emoji} <span class="p-badge ${p.badge}">${p.badge.toUpperCase()}</span> <button class="p-wish ${isWish ? 'active' : ''}" onclick="event.stopPropagation(); toggleWish(${p.id}, this)" title="${isWish ? 'Hapus dari wishlist' : 'Tambah ke wishlist'}"> ${isWish ? '❤️' : '🤍'} </button> </div> <div class="p-body"> <div class="p-cat">${p.cat}</div> <div class="p-name">${p.name}</div> <div class="p-price">${fmt(p.price)}</div> <div class="p-orig">${fmt(p.orig)}</div> <div class="p-disc">Hemat ${d}%</div> <div class="p-stats"> <span>⭐ ${p.rating}</span> <span>Terjual ${formatSold(p.sold)}</span> </div> <button class="btn-add" onclick="event.stopPropagation(); addToCart(${p.id})">+ Keranjang</button> </div>`;
...
displayCount < activeProducts.length ? ‘block’ : ‘none’;
}
...
const firstRadio = document.querySelector(‘input[name=“ratingFilter”]’);
if (firstRadio) firstRadio.checked = true;
activeCat = ‘Semua’;
document.querySelectorAll(’.cat-link’).forEach((e, i) => {
...
cart.push({ id: p.id, emoji: p.emoji, name: p.name, price: p.price, qty: 1 });
}
saveCart();
toast(`🛒 <strong>${p.name.substring(0, 28)}...</strong> ditambahkan!`);
}
...
body.innerHTML = cart.map(i => `<div class="cart-item"> <div class="ci-img">${i.emoji}</div> <div class="ci-info"> <div class="ci-name" title="${i.name}">${i.name}</div> <div class="ci-price">${fmt(i.price)} <span class="ci-qty">×${i.qty}</span></div> <div class="qty-ctrl"> <button class="qty-btn" onclick="changeQty(${i.id}, -1)" aria-label="Kurangi">−</button> <span class="qty-num">${i.qty}</span> <button class="qty-btn" onclick="changeQty(${i.id}, 1)" aria-label="Tambah">+</button> </div> </div> <button class="btn-rm" onclick="removeFromCart(${i.id})" title="Hapus" aria-label="Hapus item">🗑</button> </div>`).join(’’);

document.getElementById(‘cartTotalText’).textContent = fmt(totalCart());
...
if (!currentUser) {
toast(‘Silakan masuk akun terlebih dahulu!’, ‘warn’);
...
const p = DB.find(x => x.id === id);
if (!p) return ‘’;
return ` <div class="cart-item"> <div class="wi-img">${p.emoji}</div> <div class="ci-info"> <div class="ci-name" title="${p.name}">${p.name}</div> <div class="ci-price">${fmt(p.price)}</div> <button class="btn-add" style="margin-top:10px;padding:8px;" onclick="addToCart(${p.id})">🛒 Tambah Keranjang</button> </div> <button class="btn-rm" onclick="toggleWish(${p.id}, null); renderWishlist();" title="Hapus dari wishlist">✕</button> </div>`;
}).join(’’);
}
...
const reviewHTML = reviews.map(r => ` <div class="review-item"> <div class="review-user">${r.user} <span style="color:#F59E0B">${r.stars}</span></div> <div class="review-text">"${r.text}"</div> </div>`).join(’’);

document.getElementById(‘qvContent’).innerHTML = ` <button class="modal-close" onclick="closeModal('qvModal')">✕</button> <div class="qv-img-wrap">${p.emoji}</div> <div class="qv-right-col"> <div class="qv-cat">${p.cat}</div> <div class="qv-name">${p.name}</div> <div class="qv-pricing"> <span class="qv-price">${fmt(p.price)}</span> <div> <div class="qv-orig">${fmt(p.orig)}</div> <div class="qv-disc">Hemat ${d}%</div> </div> </div> <div class="qv-meta"> <div>⭐ <strong>${p.rating}</strong> <span>Rating</span></div> <div>📦 <strong>${formatSold(p.sold)}</strong> <span>Terjual</span></div> </div> <div class="qv-desc">${p.desc}</div> <div class="reviews"> <div class="review-title">💬 Ulasan Pembeli</div> ${reviewHTML} </div> <div class="qv-actions"> <button class="btn-qv-cart" onclick="addToCart(${p.id}); closeModal('qvModal')">🛒 Tambah ke Keranjang</button> <button class="btn-qv-wish ${isWish ? 'active' : ''}" onclick="toggleWish(${p.id}, this)" title="Wishlist">${isWish ? '❤️' : '🤍'}</button> </div> </div>`;
...
// Reset payment method to WhatsApp
selectedPay = ‘whatsapp’;
document.querySelectorAll(’.pay-opt’).forEach(el => el.classList.remove(‘selected’));
const firstPayOpt = document.getElementById(‘payOptWA’);
if (firstPayOpt) firstPayOpt.classList.add(‘selected’);
const firstRadio = document.querySelector(‘input[name=“payMethod”][value=“whatsapp”]’);
if (firstRadio) firstRadio.checked = true;
...
document.getElementById(‘ckItems’).innerHTML = cart.map(i => ` <div class="sum-item"> <span style="overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:60%;">${i.qty}× ${i.name}</span> <span style="font-weight:700;flex-shrink:0;">${fmt(i.price * i.qty)}</span> </div>`).join(’’);
...
if (cleanPhone.length < 8) { toast(‘Nomor HP tidak valid!’, ‘error’); return; }

const total   = totalCart() + SERVICE_FEE;
pendingOrder  = { name, phone, address, note, total };
...
function buildWAMessage(name, phone, address, note, total, method) {
const payLabels = {
whatsapp : ‘Transfer Bank / E-Wallet’,
qris     : ‘QRIS (Sudah Bayar)’,
cod      : ‘Bayar di Tempat (COD)’,
};
const headers = {
whatsapp : ‘🛒 PESANAN BARU — ShopNow’,
qris     : ‘💳 PEMBAYARAN QRIS — ShopNow’,
cod      : ‘📦 PESANAN COD — ShopNow’,
};

const lines = [
`*${headers[method] || '🛒 PESANAN BARU — ShopNow'}*`,
‘─────────────────────’,
‘’,
];
...
lines.push(`*Biaya Layanan:* ${fmt(SERVICE_FEE)}`);
...
lines.push(’’);
lines.push(`*💳 Metode Bayar:* ${payLabels[method] || method}`);
lines.push(‘─────────────────────’);
...
const msg = buildWAMessage(name, phone, address, note, total, ‘whatsapp’);
const url = `https://wa.me/${WHATSAPP_NO}?text=${encodeURIComponent(msg)}`;

closeModal(‘checkoutModal’);
clearCart();

document.getElementById(‘successMsg’).innerHTML = `<strong>${name}</strong>, pesanan Anda akan segera dikonfirmasi.<br> WhatsApp terbuka otomatis untuk mengirim detail pesanan ke admin kami.<br><br> Jika tidak terbuka, <a href="${url}" target="_blank" style="color:var(--primary);font-weight:800;">klik di sini</a>.`;
...
const msg = buildWAMessage(name, phone, address, note, total, ‘cod’);
const url = `https://wa.me/${WHATSAPP_NO}?text=${encodeURIComponent(msg)}`;

closeModal(‘checkoutModal’);
clearCart();

document.getElementById(‘successMsg’).innerHTML = `<strong>${name}</strong>, pesanan COD Anda berhasil dibuat!<br> Detail pesanan dikirim ke WhatsApp admin untuk segera diproses.<br><br> Bayar saat barang tiba di tangan Anda 🚚`;
openModal(‘successModal’);
setTimeout(() => window.open(url, ‘_blank’), 800);
...
const { name, phone, address, note, total } = pendingOrder;
const msg = buildWAMessage(name, phone, address, note, total, ‘qris’);
const url = `https://wa.me/${WHATSAPP_NO}?text=${encodeURIComponent(msg)}`;
...
document.getElementById(‘successMsg’).innerHTML = `<strong>${name}</strong>, terima kasih telah membayar via QRIS!<br> WhatsApp terbuka untuk mengirim bukti pembayaran ke admin kami.<br><br> Pesanan akan segera diproses setelah konfirmasi. 📦`;
openModal(‘successModal’);
setTimeout(() => window.open(url, ‘_blank’), 800);
...
function init() {
if (currentUser) {
document.getElementById(‘userLabel’).textContent = currentUser.split(’ ’)[0];
}
document.getElementById(‘cartBadge’).textContent = totalItems();
document.getElementById(‘wishBadge’).textContent = wishlist.length;
initCountdown();
